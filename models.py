...
class MLP(nn.Module):
    def __init__(self, n_in,
        ...
        self.net = nn.Sequential(*layers) # *[1, 2, 3] = 1, 2, 3 **{1, 2, 3} = 1, 2, 3
...
class GaussianActivation(nn.Module):
    ...
    def forward(self, x):   # e^(-0.5x^2)/(a^2)
...
class MultiQuadraticActivation(nn.Module):
    ...
    def forward(self, x):   # 1/sqrt(1+(ax)^2)
...
class SuperGaussianActivation(nn.Module):
    ...
    def forward(self, x):   # [e^(-0.5x^2/a^2)]^b
